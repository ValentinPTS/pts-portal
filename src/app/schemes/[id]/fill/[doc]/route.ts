...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string; doc: string }> }) {
  ...
  const pdfScript = `<script>(function(){
    ...
    b.addEventListener('click',function(){
      ...
      fetch('/api/pdf',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:${JSON.stringify(id)},doc:${JSON.stringify(doc)},lang:${JSON.stringify(lang)},composed:false})})
